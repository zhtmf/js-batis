...
class Mapper1 {
	...
	// passing js string directly to CONCAT seems to make pg-node raises "Could not determine the data type for parameter xxx" error
	@Select(`
 		select 
 			*
 		from 
 		...
 		{{#where}}
 			{{#$if 'title'}}
 				and film_title like concat('%', {{sql title}}::varchar, '%')
 			{{/$if}}
 		{{/where}}
 	`)
...
describe("pg", function() {
	...
	/*
		node-postgres by default returns all values as their string representation and custom type conversion requires another package node-pg-types,
		if we do custom type conversion specially for pg, then a question arises that whether we should do the same conversion equally to all db implementations, even 
		for some implementations that do not provide enough information for type conversion.
	 */

	it("select one row with @Select", async function() {
		this.timeout(3000);
	...
	it("select one row with @SelectOne", async function() {
		this.timeout(3000);
	...
	it("select single result", async () => {
		...
		expect(results).to.be.not.a('array');
