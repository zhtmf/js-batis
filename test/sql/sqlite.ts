...
class Mapper1 {
	...
	@Select(`
 		SELECT
 			f.film_id,
 			f.film_title,
 			SUBSTRING(film_title, length(film_title), 1) as str_num,
 			f.release_year,
 			f.category,
 			a.actor_id as actor_actor_id,
 			a.first_name as actor_first_name,
...
async function prepare(mapper1) {

	const sqlite3 = require("sqlite3");
	const db = new sqlite3.Database('sqlite_test_db');

	let promises: (() => Promise<void>)[] = [];
	let makePromise = function(sql) {
		return function() {
	...
	promises.push(makePromise("DROP TABLE IF EXISTS language"));
	promises.push(makePromise(`CREATE TABLE IF NOT EXISTS language (
		  language_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
		  name TEXT NOT NULL
		)`));

	promises.push(makePromise("DROP TABLE IF EXISTS film_language"));
	promises.push(makePromise(`CREATE TABLE IF NOT EXISTS film_language (
	  film_id INTEGER NOT NULL,
	  language_id INTEGER NOT NULL ,
	  PRIMARY KEY(film_id,language_id)
	)`));

	promises.push(makePromise("DROP TABLE IF EXISTS film_actor"));
	promises.push(makePromise(`CREATE TABLE IF NOT EXISTS film_actor (
	  film_id INTEGER NOT NULL,
	  actor_id INTEGER NOT NULL,
	  PRIMARY KEY(film_id,actor_id)
	)`));

	promises.push(makePromise("DROP TABLE IF EXISTS film"));
	promises.push(makePromise(`CREATE TABLE IF NOT EXISTS film (
	  film_id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
	  film_title TEXT NOT NULL,
	  release_year UNSIGNED INTEGER NOT NULL,
	  category TEXT NOT NULL
	) `));

	promises.push(makePromise("DROP TABLE IF EXISTS actor"));
	promises.push(makePromise(`CREATE TABLE IF NOT EXISTS actor (
	...
	promises.push(makePromise("DROP TABLE IF EXISTS insert_test"));
	promises.push(makePromise(`CREATE TABLE insert_test (
	  id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
	  name TEXT NOT NULL
	)  `));

	promises.push(makePromise("DROP TABLE IF EXISTS insert_test2"));
	// make the id not auto incremental
	// unsigned integer in SQLite is not an alias for rowid
	promises.push(makePromise(`CREATE TABLE insert_test2 (
	  id INTEGER PRIMARY KEY NOT NULL,
	  name TEXT NOT NULL
	) `));

	promises.push(makePromise("INSERT OR IGNORE INTO language (language_id, name) VALUES (1, 'Language1') "));
	promises.push(makePromise("INSERT OR IGNORE INTO language (language_id, name) VALUES (2, 'Language2') "));
	promises.push(makePromise("INSERT OR IGNORE INTO language (language_id, name) VALUES (3, 'Language3') "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_language (film_id, language_id) VALUES (1, 1) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_language (film_id, language_id) VALUES (1, 2) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_language (film_id, language_id) VALUES (2, 3) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_language (film_id, language_id) VALUES (3, 2) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_language (film_id, language_id) VALUES (3, 3) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_language (film_id, language_id) VALUES (2, 1) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_actor (film_id, actor_id) VALUES (1, 1) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_actor (film_id, actor_id) VALUES (2, 1) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_actor (film_id, actor_id) VALUES (2, 2) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film_actor (film_id, actor_id) VALUES (2, 3) "));
	promises.push(makePromise("INSERT OR IGNORE INTO film (film_id, film_title, release_year, category) VALUES (1, 'Film1', 1001, 'category1') "));
	promises.push(makePromise("INSERT OR IGNORE INTO film (film_id, film_title, release_year, category) VALUES (2, 'Film2', 1002, 'category1') "));
	promises.push(makePromise("INSERT OR IGNORE INTO film (film_id, film_title, release_year, category) VALUES (3, 'Film3', 1003, 'category2') "));
	promises.push(makePromise("INSERT OR IGNORE INTO actor (actor_id, first_name, last_name, active) VALUES (1, 'First1', 'Last1', 0) "));
	promises.push(makePromise("INSERT OR IGNORE INTO actor (actor_id, first_name, last_name, active) VALUES (2, 'First2', 'Last2', 1) "));
	promises.push(makePromise("INSERT OR IGNORE INTO actor (actor_id, first_name, last_name, active) VALUES (3, 'First3', 'Last3', 0) "));
...
describe("sqlite", function() {
	...
	before(function(done) {
		this.timeout(10000);
		prepare(mapper1).then(function(val) {
			done();
		}).catch(function(e) {
			done(e);
		});
	...
	it("insert, return auto generated id", async () => {
		//AUTOINCREMENT make it very slow
		this.timeout(4000);
		for (let i = 0; i < 5; ++i) {
			let results2 = await mapper1.insertTest(undefined, 'name' + i);
			expect(results2).to.be.equals(i + 1);
			logContains(loggerSpy, `generated key: ${results2}`);
		}
	});

	it("insert, no generated id", async () => {
		let results2 = await mapper1.insertTest2(999, 'name999');
		expect(results2).to.be.equals(999);
		//WARNING: A feature in sqlite, on an insert even the rowid alias is explicitly given a value, last_insert_row_id() will return that value rather than 0 
		logContains(loggerSpy, `generated key: 999`);
	});
	...
	it("new transaction embedded in existing transaction for the same data source", async () => {
		this.timeout(3000);
	...
	//in sqlite, a database can only be accessed by one transaction at a time as a database is really a single file
